...
with tf.device(device):
    ...
    for i, data in enumerate(base_generator):
        ...
        if i%1==0:
            ...
            vxm.py.utils.save_volfile(p_zone, os.path.join(save_file, '%s_dice_%.4f_pz.nii.gz' % (name[0].split('.')[0], accuracy)))
            vxm.py.utils.save_volfile(t_zone, os.path.join(save_file, '%s_dice_%.4f_tz.nii.gz' % (name[0].split('.')[0], accuracy)))
