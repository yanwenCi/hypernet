...
class HyperUnetDense(ne.modelio.LoadableModel):
    ...
    def __init__(self,
        ...
        super().__init__(name=name, inputs=[source1, source2, source3, hyp_input], outputs=[outputs
          ,output_list[0],output_list[1], output_list[2],
                                                                                            ])
...
##############################################################################
#
class weight_sum(tf.keras.layers.Layer):
    def __init__(self, dim):
        super(weight_sum, self).__init__()
        self.ndims = dim

    def call(self, I, hyper):
        bias = None
        hyper=hyper[0, ...]
        if hyper.shape.as_list()[0] == self.ndims + 1:
            bias = hyper[3:]
            hyper = hyper[:3]

        # compute filters ks*ks*ks*inch*outch
        sum_filt1=hyper
        for _ in range(self.ndims):
            sum_filt1 = tf.expand_dims(sum_filt1, 0)
        sum_filt1 = tf.expand_dims(sum_filt1, -1)

        # get convolution function
        conv_fn = getattr(tf.nn, 'conv%dd' % self.ndims)
        strides = 1
        if self.ndims > 1:
            strides = [1] * (self.ndims + 2)
        # compute CC squares
        I_sum = conv_fn(I, sum_filt1, strides, padding='SAME')
        if bias is not None:
            I_sum = tf.nn.bias_add(I_sum, bias)
        return I_sum
