...
class UNet(tf.keras.Model):
    ...
    def call(self, x, training=True):
        ...
        input_shape = self.in_shape#(int(x.shape[1]), int(x.shape[2]), int(x.shape[3]))
...
class NNUnet(tf.keras.Model):
    def __init__(self, in_shape, n_channels, n_classes, pocket, deep_supervision):
        super(NNUnet, self).__init__()
        kernels, strides = self.get_unet_params()
        self.n_classes = n_classes
        #input_shape = (None, None, None, n_channels)
        input_shape = (None,) + in_shape +(n_channels,)
    ...
    @staticmethod
    def get_unet_params():
        patch_size, spacings = [64,64,64], [1,1,1]#config["patch_size"], config["target_spacing"]
